...
class Configuration(object):
    """
    Configuration provides a wrapper around a user's andsploit configuration, stored
    in their home directory as .andsploit_config.
    """
    ...
    def library(cls, name):
        """
        Returns the path to a andsploit Library
        """
    ...
    def path(cls):
        ...
        if os.path.exists(os.path.sep.join([".", ".andsploit_config"])):
            return os.path.sep.join([".", ".andsploit_config"])
        else:
            return os.path.sep.join([os.path.expanduser("~"), ".andsploit_config"])
