...
class Package(Module, common.Assets, common.ClassLoader, common.Filters, common.PackageManager):

    name = "Automatically send intents with correct action, data, or other intent-filters to services and receivers"
    ...
    def execute(self, arguments):
        ...
        if not arguments.package:
            for package in self.packageManager().getPackages(common.PackageManager.GET_RECEIVERS | common.PackageManager.GET_PERMISSIONS | common.PackageManager.GET_GIDS):
                print "fuzzing package %s......" % str(package.packageName)
    ...
    def __fuzz_package(self, shell, arguments, package):
        if self.__report__exists("./report", str(package.packageName)):
            print "package %s has been analyzed." % str(package.packageName)
            return
