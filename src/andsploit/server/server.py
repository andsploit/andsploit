...
from andsploit import util
from andsploit.server import dz, uploader
from andsploit.ssl.provider import Provider

class Server(cli.Base):
    """
    andsploit console COMMAND [OPTIONS]
    
    Starts a new andsploit Server, or runs utilities to interact with a running
    Server.

    The andsploit Server accepts connections from agents, and routes sessions
    ...
    def do_delete(self, arguments):
        """delete a resource from the andsploit Server"""
    ...
    def args_for_delete(self):
        self._parser.add_argument("resource", help="specify a resource to upload to a andsploit Server")
    ...
    def do_upload(self, arguments):
        """upload a resource to the andsploit Server"""
    ...
    def args_for_upload(self):
        self._parser.add_argument("resource", help="specify a resource to upload to a andsploit Server")
        self._parser.add_argument("file", help="specify a resource to upload to a andsploit Server")
