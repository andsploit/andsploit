...
try:
    ...
    print "andsploit Server requires Twisted to run."
    print "Run 'easy_install twisted==10.2.0' to fetch this dependency."
    sys.exit(-1)

from andsploit.configuration import Configuration
from andsploit.server.heartbeat import heartbeat
from andsploit.server.protocols.byte_stream import ByteStream
from andsploit.server.protocols.shell import ShellCollector, ShellServer
from andsploit.server.files import FileProvider, FileResource
from andsploit.server.protocols.http import HTTP
from andsploit.server.protocols.andsploitp import andsploit
...
def serve(arguments):
    ...
    if arguments.ssl != None:
        print "Starting andsploit Server, listening on 0.0.0.0:%d (with SSL)" % arguments.port

        if arguments.ssl == []:
            print "Using default SSL key material..."
            
            arguments.ssl = Provider().get_keypair("andsploit-server")
        ...
        print "Starting andsploit Server, listening on 0.0.0.0:%d" % arguments.port
...
class ProtocolSwitcher(Protocol):
    ...
    protocols being spoken to the andsploit Server.

    If the incoming message starts with GET or POST, the server will route the
    connection to an HTTP server, otherwise it is connected to the andsploit
    Server.
    """
    ...
    __file_provider = FileProvider({ "/": FileResource("/", path.join(__web_root, "index.html"), magic="I", reserved=False, type="text/html"),
                                     ...
                                     "/andsploit.png": FileResource("/andsploit.png", path.join(__web_root, "andsploit.png"), reserved=True, type="image/png"),
