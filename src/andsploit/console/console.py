...
from andsploit.api.formatters import SystemResponseFormatter
from andsploit.connector import ServerConnector
from andsploit.console.session import Session, DebugSession

class Console(cli.Base):
    """
    andsploit console [OPTIONS] COMMAND
    
    Starts a new andsploit Console to interact with an Agent.

    The andsploit Console connects to an Agent and allows you to interact with the
    ...
    a andsploit Server that the agent is connected to.
    """
    ...
    def __init__(self):
        ...
        self._parser.add_argument("--no-color", action="store_true", default=False, help="disable syntax highlighting in andsploit output")
    ...
    def do_devices(self, arguments):
        """lists all devices bound to the andsploit server"""
    ...
    def do_disconnect(self, arguments):
        """disconnects a andsploit session"""
    ...
    def handle_error(self, throwable, fatal=False):
        ...
        if str(throwable) == "connection reset by peer":
            sys.stderr.write("The andsploit Server is not available. Please double check the address and port.\n\n")
            ...
            sys.stderr.write("There was a problem connecting to the andsploit Server.\n\n")
            sys.stderr.write("Things to check:\n\n")
            sys.stderr.write(" - is the andsploit Server running?\n")
    ...
    def __manage_trust(self, provider, certificate, peer):
        ...
        if trust_status < 0:
            ...
            print "andsploit has established an SSL Connection to %s:%d." % peer
